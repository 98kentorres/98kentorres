<?php include('configRequests.php');
...
<body>
    ...
    if($position == "CASHIER"){
        ...
        $_SESSION['message'] = "CASHIERS ARE NOT ALLOWED TO VIEW REQUEST PANEL";
...
<br><br>
<div class="dboard">
    <p class="dboard-text"><img class="dboard-img"src="img/black-db.png"><b>Dashboard</b> / <i class="lets">Requests</i></p>
        </div>
    <br>
...
<div class="box-tbl">
  <table>
    <thead>
        <tr><th class="th-css">ACTION</th>
            <th class="th-css">CASHIERS' REQUESTS</th>
            <th class="th-css">REQUESTED BY</th>
            <th class="th-css">REQUESTED DATE/TIME</th>
        </tr>
    </thead>
      <tbody>
        <?php $notifs = mysqli_query($db, "SELECT * FROM notifs ORDER BY id DESC");
        while($row = mysqli_fetch_array($notifs)):?>
          <tr>
            <td><center>
              <a href="requests.php?delete=<?php echo $row['id']; ?>"><img src="img/stop.png" title="REMOVE"></a>&nbsp&nbsp&nbsp
              <a href="requests.php?save=<?php echo $row['id']; ?>"><img src="img/correct.png" title="REQUEST ACCEPTED"></a>
            </center></td>
              <td class="activity-font"><center><?php echo $row['notes'];?></center></td>
              <td class="activity-font"><center><?php echo $row['requested_by'];?></center></td>
  ...
  <div class="center-numbers">
           <?php
               ...
               if($page>1){
                   echo "<a href='requests.php?page=".($page-1)."'class='btn btn-default'>← Prev</a>";

               }

               for($i=1;$i<$total_page;$i++){
                //if the total page reached 10 student data (CURRENT TOTAL PAGE IS 2), new number button will be appear which is 3
                   echo "<a href='requests.php?page=".$i."'class='btn btn-default btn-css'>$i</a>";
               }

               if($i>$page){
                   echo "<a href='requests.php?page=".($page+1)."'class='btn btn-primary btn-css'>Next →</a>";
